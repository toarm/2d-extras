    ...
    public class PrefabBrush : GridBrush {
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position) {
            ...
            Vector3 gridPosition =
                grid.CellToLocalInterpolated(new Vector3Int(position.x, position.y, m_Z) + new Vector3(.5f, .5f, .5f));
            if (!PrefabAlreadyExists(prefab, gridPosition)) {
        ...
        bool PrefabAlreadyExists(GameObject prefab, Vector3 position) {
            RaycastHit2D hit = Physics2D.Raycast(new Vector2(position.x, position.y), Vector2.zero);
            if (hit.collider != null) {
                return hit.collider.gameObject == prefab;
            }
        ...
        public override void Erase(GridLayout grid, GameObject brushTarget, Vector3Int position) {
            ...
            Transform erased =
                GetObjectInCell(grid, brushTarget.transform, new Vector3Int(position.x, position.y, m_Z));
            if (erased != null)
